...
<template>
  <SystemMessage v-if="type === 'system'" :content="content.text" :action="content.action" />
  <div v-else class="Message" :class="[position]" :data-id="id" :data-type="type">
    <div v-if="hasTime && createdAt" class="Message-meta">
      <Time :date="createdAt" />
    </div>
    <div class="Message-main">
      <Avatar v-if="isRL && user?.avatar">
        <AvatarImage :src="user.avatar" :alt="user.name" :url="user.url" />
      </Avatar>
      <div class="Message-inner">
        <div v-if="isRL && user?.name" class="Message-author">
          {{ user.name }}
        </div>
