<script lang="ts">
import type { MessageProps } from '@/composables/useMessages'
...
<template>
  <div ref="messagesRef" class="MessageContainer" tabindex="-1">
    <div class="MessageContainer-before">
      <slot name="before" />
    </div>
    <div ref="scrollerRef" class="MessageList" @scroll="handleScroll">
      <slot />
      <Message v-if="isTyping" id="typing" type="typing" />
    </div>
    <BackBottom
      v-if="showBackBottom"
      :count="newCount"
      @click="handleBackBottomClick"
      @did-mount="onBackBottomShow"
    />
  </div>
</template>
